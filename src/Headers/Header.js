...
const Header = () => {
    ...
    const handleSuccessGoogle = async (credentialResponse) => {
        try {
            ...
            const { patient_name, patient_id } = response.data; // Đổi từ 'username' sang 'patient_username'
            if (patient_name && patient_id) {
                setPatientId(patient_id);
                sessionStorage.setItem('patient_name', patient_name); // Lưu 'patient_username' thay vì 'username'
                sessionStorage.setItem('patient_id', patient_id);
                setUsername(patient_name); // Đặt 'patient_username'
                sessionStorage.setItem('loginMessage', 'Sign in successfully');
                setIsLoggedIn(true);
